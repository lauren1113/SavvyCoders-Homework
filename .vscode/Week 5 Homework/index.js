...
userData.forEach(user => console.log(user.phone));

//  create and print a list of website and email pairs (pair them in a string, array, or object)
const jargon = userData.map(user => `${user.website} - ${user.email}`);
console.log(jargon);
...
let tallyObject = toDoData.reduce(
    (tallyObject, curr) => {
        if (curr.completed === true) {
            tallyObject.completedTasks += 1;
            return tallyObject;
        }
    tallyObject.incompleteTasks += 1;
    return tallyObject;
    },
    {
        completedTasks: 0,
...
const userTasksSummary = toDoData.reduce((acc, curr) => {
    if (!acc[curr.userId]) {
        acc[curr.userId] = {
            totalTasks: 1
        };
        if (curr.completed) {
            acc[curr.userId].completedTasks = 1;
            acc[curr.userId].incompleteTasks = 0;
            return acc;
        }
        acc[curr.userId].completedTasks = 0;
